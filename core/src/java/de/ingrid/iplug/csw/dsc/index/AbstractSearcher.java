...
/**
 * The IPlug searcher interface that all new dsc variant IPlugs have to be implemented.
 */
public abstract class AbstractSearcher implements IPlug, IRecordLoader {
    ...
    public abstract void configure(PlugDescription plugDescription) throws Exception;

    public abstract IngridHits search(IngridQuery query, int start, int length) throws Exception;
     ...
     * @param addDataTypes
     *            True when the datatypes sahould be on the created lucene query.
     * @param start
     ...
     *             If something goes wrong.
     */
    public IngridHits search(IngridQuery query, boolean addDataTypes, int start, int length) throws Exception {
        checkLock();
        if (this.fSearcher == null) {
            throw new IllegalArgumentException("searcher can not be null, plug not correct configured.");
        }
        if (fTitleBoost == -1) {
            setTitleBoost();
    ...
    private void setTitleBoost() {
        if (getPlugDescription() == null) {
            fTitleBoost = 1.0f;
            return;
        }
        Construct construct = getConstruct(getPlugDescription());
        // skip for tamino
        if (construct != null) {
            Column[] columnsToIndex = construct.getColumnsToIndex();
            for (int i = 0; i < columnsToIndex.length; i++) {
                if ("title".equals(columnsToIndex[i].getTargetName())) {
                    fTitleBoost = columnsToIndex[i].getBoost();
                    break;
                }
            }
            if (fTitleBoost == -1) {
                throw new IllegalArgumentException("title boost not set");
            }
        }
    }

    private void addGroupingInformation(IngridHit ingridHit, IngridQuery query, int count) {
     ...
     *             If something goes wrong.
     */
    public static Query buildLuceneQuery(IngridQuery query, boolean addDataTypes) throws IOException {
        ...
        if (addDataTypes) {
            ...
            for (int i = 0; i < fields.length; i++) {
                ...
                if (fieldQuery.isRequred() && fieldQuery.isProhibited()) {
                    booleanQuery.add(new org.apache.lucene.search.TermQuery(new Term(fieldQuery.getFieldName(),
                            fieldQuery.getFieldValue().toLowerCase())), false, true);
                } else {
                    booleanQuery.add(new org.apache.lucene.search.TermQuery(new Term(fieldQuery.getFieldName(),
                            fieldQuery.getFieldValue().toLowerCase())), fieldQuery.isRequred(), fieldQuery
                            .isProhibited());
                }
            }
        }
        ...
        for (int i = 0; i < terms.length; i++) {
            ...
            if (term.indexOf(' ') > -1) {
                ...
                if (term.endsWith("*")) {
                    String filteredTerm = filterTerm(term.substring(0, term.length() - 1));
                    if (filteredTerm.indexOf(' ') > -1) {
                        filteredTerm = "\"" + filteredTerm + "\"";
                    }
                    if (termQuery.isProhibited() && termQuery.isRequred()) {
                        booleanQuery.add(new PrefixQuery(new Term(CONTENT, filteredTerm)), false, true);
                    } else {
                        booleanQuery.add(new PrefixQuery(new Term(CONTENT, filteredTerm)), termQuery.isRequred(),
                                termQuery.isProhibited());
                    }
                    PrefixQuery titlePrefixQuery = new PrefixQuery(new Term("title", filteredTerm));
                    ...
                    if (filteredTerm.indexOf(' ') > -1) {
                        ...
                        if (termQuery.isProhibited() && termQuery.isRequred()) {
                            booleanQuery.add(new org.apache.lucene.search.TermQuery(new Term(CONTENT, filteredTerm)),
                                    false, true);
                        } else {
                            booleanQuery.add(new org.apache.lucene.search.TermQuery(new Term(CONTENT, filteredTerm)),
                                    termQuery.isRequred(), termQuery.isProhibited());
                        }
        ...
        for (int i = 0; i < clauses.length; i++) {
            ...
            if (!sc.equals(fBOOLEANQUERYCOMPARATOROBJECT)) {
                if (clauseQuery.isProhibited() && clauseQuery.isRequred()) {
                    ...
                    booleanQuery.add(sc, clauseQuery.isRequred(), clauseQuery.isProhibited());
                }
            }
        }
        ...
        for (int i = 0; i < rangeQueries.length; i++) {
            ...
            Term termFrom = new Term(rangeQuery.getRangeName(), rangeQuery.getRangeFrom().toLowerCase());
            Term termTo = new Term(rangeQuery.getRangeName(), rangeQuery.getRangeTo().toLowerCase());
            org.apache.lucene.search.RangeQuery luceneQuery = new org.apache.lucene.search.RangeQuery(termFrom, termTo,
                    rangeQuery.isInclusive());
        ...
        for (int i = 0; i < wildCardFieldQueries.length; i++) {
            ...
            WildcardQuery luceneQuery = new WildcardQuery(new Term(wildCardQuery.getFieldName(), wildCardQuery
                    .getFieldValue()));
        ...
        for (int i = 0; i < wildCardTermQueries.length; i++) {
            WildcardQuery luceneQuery = new WildcardQuery(new Term(CONTENT, wildCardTermQueries[i].getTerm()));
            if (wildCardTermQueries[i].isRequred() && wildCardTermQueries[i].isProhibited()) {
                booleanQuery.add(luceneQuery, false, true);
            } else {
                booleanQuery
                        .add(luceneQuery, wildCardTermQueries[i].isRequred(), wildCardTermQueries[i].isProhibited());
            }
        }
        ...
        for (int i = 0; i < fuzzyFieldQueries.length; i++) {
            ...
            FuzzyQuery fuzzyQuery = new FuzzyQuery(new Term(fuzzyFieldQuery.getFieldName(), fuzzyFieldQuery
                    .getFieldValue()));
            booleanQuery.add(fuzzyQuery, fuzzyFieldQuery.isRequred(), fuzzyFieldQuery.isProhibited());
        }
        ...
        for (int i = 0; i < fuzzyTermQueries.length; i++) {
            ...
            FuzzyQuery fuzzyQuery = new FuzzyQuery(new Term(CONTENT, fuzzyTermQuery.getTerm()));
            if (fuzzyTermQuery.isRequred() && fuzzyTermQuery.isProhibited()) {
                booleanQuery.add(fuzzyQuery, false, true);
            } else {
                booleanQuery.add(fuzzyQuery, fuzzyTermQuery.isRequred(), fuzzyTermQuery.isProhibited());
            }
        }
    ...
    private static void addPhraseQuery(BooleanQuery booleanQuery, TermQuery termQuery, String term, float titleBoost)
            throws IOException {
        ...
        if (termQuery.isProhibited() && termQuery.isRequred()) {
            ...
            booleanQuery.add(phraseQuery, termQuery.isRequred(), termQuery.isProhibited());
        }
     ...
     * @param booleanQuery
     */
    private static void processGeoAndTimeQueries(FieldQuery[] fields, BooleanQuery booleanQuery) {
        ...
        for (int i = 0; i < fields.length; i++) {
            ...
            if (indexField.equals("x1")) {
                ...
                booleanQuery.add(new org.apache.lucene.search.TermQuery(new Term(query.getFieldName(), query
                        .getFieldValue().toLowerCase())), false, false);
            } else {
                ...
                if (term.indexOf(' ') > -1) {
                    ...
                    booleanQuery.add(phraseQuery, query.isRequred(), query.isProhibited());
                } else {
                    booleanQuery.add(new org.apache.lucene.search.TermQuery(new Term(field, term)), query.isRequred(),
                            query.isProhibited());
                }
            }
        }
    ...
    public IngridHitDetail getDetail(IngridHit hit, IngridQuery ingridQuery, String[] fields) throws Exception {
        ...
        try {
            if (this.fUrl != null) {
                detail.put("url", constructDetailUrl(this.fUrl, luceneDocument));
            } else {
    ...
    private void addPlugDescriptionInformations(IngridHitDetail detail, String[] fields) {
        for (int i = 0; i < fields.length; i++) {
            if (fields[i].equals(PlugDescription.PARTNER)) {
                detail.setArray(PlugDescription.PARTNER, getPlugDescription().getPartners());
            } else if (fields[i].equals(PlugDescription.PROVIDER)) {
                detail.setArray(PlugDescription.PROVIDER, getPlugDescription().getProviders());
            }
        }
    ...
    public abstract Record getRecord(IngridHit hit) throws IOException, Exception;
    ...
    /**
     * Returns the construct of hierachical database mapping.
     * 
     * @param plugDescription
     * @return The construct.
     */
    public static Construct getConstruct(PlugDescription plugDescription) {
        return (Construct) plugDescription.get("construct");
    }
     ...
     */
    public static String constructDetailUrl(String url, Document document) throws IllegalArgumentException {
    ...
    private static void prepareIncludeGeoQuery(BooleanQuery booleanQuery, Map geoMap) {
    ...
    private static void prepareExactGeoQuery(BooleanQuery booleanQuery, Map geoMap) {
        String x1 = (String) geoMap.get("x1");
        String x2 = (String) geoMap.get("x2");
        String y1 = (String) geoMap.get("y1");
        String y2 = (String) geoMap.get("y2");
     ...
     * @param geoMap
     */
    private static void prepareIntersectGeoQuery(BooleanQuery booleanQuery, Map geoMap) {
        String x1 = (String) geoMap.get("x1");
        String x2 = (String) geoMap.get("x2");
        String y1 = (String) geoMap.get("y1");
        String y2 = (String) geoMap.get("y2");
    ...
    private static void prepareInsideGeoQuery(BooleanQuery booleanQuery, Map geoMap) {
        String x1 = (String) geoMap.get("x1");
        String x2 = (String) geoMap.get("x2");
        String y1 = (String) geoMap.get("y1");
        String y2 = (String) geoMap.get("y2");
    ...
    private static void prepareInsideOrIncludeQuery(BooleanQuery query, Map timeMap) {
    ...
    private static void prepareInsideOrIntersectTime(BooleanQuery query, Map timeMap) {
        BooleanQuery booleanQueryTime = new BooleanQuery();
        BooleanQuery inside = new BooleanQuery();
    ...
    private static void prepareInsideTime(BooleanQuery query, Map timeMap) {
        ...
        if (t1 != null && t2 != null) {
            ...
            org.apache.lucene.search.RangeQuery rangeQueryt0 = new org.apache.lucene.search.RangeQuery(termT0Min,
                    termT0Max, true);
            org.apache.lucene.search.RangeQuery rangeQuery11 = new org.apache.lucene.search.RangeQuery(termT1Min, termT1Max,
                    true);
            org.apache.lucene.search.RangeQuery rangeQuery12 = new org.apache.lucene.search.RangeQuery(termT2Min, termT2Max,
                    true);
            ...
            query.add(new org.apache.lucene.search.TermQuery(termT0), true, false);
        }
    }
    ...
    private static void prepareTraverseTime(BooleanQuery query, Map timeMap) {
        ...
        if (t1 != null && t2 != null) {
            ...
            // (ti2:[tq1 TO tq2] && ti1:[00000000 TO tq1]) || (ti1:[tq1 TO tq2] && ti2:[tq2 TO 99999999])
            Term termT1Min = new Term("t1", "00000000");
            ...
            BooleanQuery booleanQueryTime = new BooleanQuery();
            booleanQueryTime.add(new org.apache.lucene.search.TermQuery(termT0), false, false);
            booleanQueryTime.add(new org.apache.lucene.search.TermQuery(termT1), false, false);
            booleanQueryTime.add(new org.apache.lucene.search.TermQuery(termT2), false, false);
            query.add(booleanQueryTime, true, false);
        }
    }
}
